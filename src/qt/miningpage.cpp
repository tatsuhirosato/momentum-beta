...
void MiningPage::startPoolMining()
{
    ...
    args << "--scrypt";
    args << "--scan-time" << ui->scantimeBox->text().toAscii();
    args << "-o" << urlLine.toAscii();
    args << "-u" << ui->usernameLine->text().toAscii();
    args << "-p" << ui->passwordLine->text().toAscii();
    args << "-I" << ui->threadsBox->text().toAscii();
    args << "" << ""; // Retry forever.
    args << ""; // This is needed for this to work correctly on Windows. Extra protocol dump helps flush the buffer quicker.
    ...
    // If CGminer is in current path, then use that. Otherwise, assume CGminer is in the path somewhere.
...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList("Miner failed to start. Make sure you have the CGminer executable and libraries in the same directory as momentum-beta-qt.", ERROR, NULL);
    }
}
